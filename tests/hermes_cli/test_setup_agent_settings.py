...
from hermes_cli.setup_wizard import setup_agent_settings
...
def test_setup_agent_settings_uses_displayed_max_iterations_value(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("hermes_cli.setup_wizard.prompt", lambda *args, **kwargs: next(prompt_answers))
    monkeypatch.setattr("hermes_cli.setup_wizard.prompt_choice", lambda *args, **kwargs: 4)
    monkeypatch.setattr("hermes_cli.setup_wizard.save_env_value", lambda *args, **kwargs: None)
    monkeypatch.setattr("hermes_cli.setup_wizard.remove_env_value", lambda *args, **kwargs: None)
    monkeypatch.setattr("hermes_cli.setup_wizard.save_config", lambda *args, **kwargs: None)
...
def test_setup_agent_settings_prefers_config_over_stale_env(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        "hermes_cli.setup_wizard.get_env_value",
    ...
    monkeypatch.setattr(
        "hermes_cli.setup_wizard.remove_env_value",
