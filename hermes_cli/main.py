            ...
            if current_branch not in ("main", "HEAD"):
                ...
                if restore_branch_result.returncode != 0:
                    ...
                    if auto_stash_ref is not None:
                        print(
                            f"  Local changes remain preserved in git stash (ref: {auto_stash_ref})."
                        )
                    sys.exit(1)
            if auto_stash_ref is not None:
...
""",
    ...
    _report_parser = memory_sub.add_parser(
        ...
        description=(
            "Inspect read-only daily memory maintenance reports generated by the "
            "systemd timer.\n\n"
            "Use 'hermes memory report latest' to view the newest report bundle."
        ),
    )
    _report_sub = _report_parser.add_subparsers(dest="report_command", required=True)
    _report_latest = _report_sub.add_parser(
        "latest",
        help="Show the newest read-only memory maintenance report",
        description=(
            "Show the newest daily read-only memory maintenance report bundle.\n\n"
            "This command only reads the log files under ~/.hermes/logs/memory-maintenance/."
        ),
    )
    _report_latest.add_argument(
        "--json",
        action="store_true",
        help="Emit machine-readable JSON instead of human output",
    )
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from hermes_cli.memory_report import cmd_memory_report
